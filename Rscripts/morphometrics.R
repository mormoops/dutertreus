...
setwd("/Users/Pedro/Desktop/Eptesicus_fuscus/CHAPTER.1/morphometrics")

#Upload dataset
morpho_final<-read.csv(file = "./morphodata_new.csv", header = T)
dim(morpho_final)
View(morpho_final)
...
#Extract the group names for Locality analyses
...
#ORIGINAL clrs = c("#40004b", "#1b7837", "#f03b20", "#2166ac")
colrs <- c("#40004b", "#1b7837", "#f03b20")
...
brownbatFig1density <- ggplot(brownbat_LD, aes(lda.LD1, fill = type)) + 
  ...
  scale_fill_manual(values = c("#40004b", "#1b7837", "#f03b20", "#2166ac"),
                    labels = c("Caribbean", "Mesoamerica", "Southeastern North America")) + 
...
################################## FOR THE SEX DIFERENCES
#This analyses use a new dataset, based on the first one 

#Upload dataset with Sex information
morpho_final<-read.csv(file = "./morphodata_new_no.NA.csv", header = T)
dim(morpho_final)
View(morpho_final)

#######
sp.sex<-morpho_final[20]
head(sp.sex)
#######
...
# Create training/testing splits
set.seed(123) #this is a random number; analysis will start at "'123" position 
training <- sp$Sex %>% createDataPartition(p = 0.75, list = F) #3/4 of total data used to training purposes 
# training set
train.sp <- sp[training, ]
dim(train.sp)
head(train.sp)
...
# Fit the LDA model to training dataset
model.fit <- train(Sex~., data = train.sp, preProcess = c("center", "scale"), method = "lda")
...
predictions <- predict(model.fit, newdata = test.sp)
# Make a table to ensure both data & reference are factors
t <- table(factor(predictions), factor(test.sp$Sex))
t
...
# Confusion matrix of LDA machine learning classification; Assess the prediction accuracy of the LDA from the model to the actual data
brownbat.lda.predict <- train(Sex ~ .,  data = sp, method = "lda", metric = performance.metric,
                              trControl = kfoldcv, preProcess = c("center", "scale"))

# calculate a confusion matrix to see the accuracy of classification
confusionMatrix(as.factor(sp$Sex), predict(brownbat.lda.predict, sp))  
...
# create stacked histograms of the LDA values for csv subspecies
brownbat.lda.values <- predict(brownbat.lda)
ldahist(brownbat.lda.values$x[ ,1], g = sp$Sex, 
        type = "histogram", col = "light gray") 
...
#NEW
var_explained = vil_pca$sdev^2 / sum(vil_pca$sdev^2)
...
pca.table<-as.data.frame(vil_pca$x)
caribbean.names.pca<-morpho_final$Sex
new.names.caribbean<-cbind(caribbean.names.pca,pca.table)

theme_set(theme_light())
colrs <- c("#40004b", "#f03b20", "#1b7837")
#colrs <- c("#40004b", "#f03b20", "#1b7837")
#"#f03b20" laranja
#"#1b7837" verde 
#"#40004b" roxo
